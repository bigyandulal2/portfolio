...
import { FiArrowLeft, FiArrowRight, FiZap, FiTarget, FiStar, FiChevronRight } from 'react-icons/fi'; // Replaced FiHeart with FiTarget, FiCode with FiZap
import { useNavigate } from 'react-router-dom';

// --- CONFIGURATION ---
// Based on your homepage screenshot:
const PRIMARY_BG_COLOR = 'bg-gradient-to-br from-teal-800 to-teal-900'; // Darker teal blend for depth
const TEXT_COLOR_LIGHT = 'text-white';
const TEXT_COLOR_DIM = 'text-gray-300'; // For secondary text
const ACCENT_COLOR_RED = 'text-rose-500'; // Using Tailwind's rose-500 for that bold red
const ACCENT_COLOR_RED_BG = 'bg-rose-500'; // For buttons/highlights
const ACCENT_COLOR_RED_HOVER_BG = 'hover:bg-rose-600'; // Slightly darker on hover
...
const skills = [
  ...
  'Express.js', 'MongoDB', 'Framer Motion', 'REST APIs'
];

// --- ANIMATION VARIANTS ---
// Slightly adjusted for a more confident, flowing reveal
...
const itemVariants = {
  ...
  visible: {
    ...
    scale: 1,
    transition: {
      type: "spring", // Use a spring for a slightly bouncier, more engaging feel
      stiffness: 100,
      damping: 12,
      mass: 0.5
    }
...
const About = () => {
  ...
  return (
    <div className={`min-h-screen ${PRIMARY_BG_COLOR} relative overflow-hidden`}>
      {/* Subtle Background Pattern: Lighter teal on dark teal */}
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 py-6">
        ...
        {/* Navigation - Matching button style from homepage */}
        <motion.nav
        ...
        {/* Header - Bold and impactful, using your homepage's font weights */}
        <motion.header
        ...
        >
          ...
          >
            More than just a FullStack Developer, I'm a creative problem-solver with a passion for building seamless and impactful digital experiences.
          </motion.p>
        </motion.header>

        {/* Stats Section - Clean, bold, and aligned with your aesthetic */}
        <motion.div
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
        ...
        >
          ...
          {/* Core Toolkit (Skills) */}
          <div>
            <motion.h2 variants={itemVariants} className={`text-3xl font-bold mb-6 ${TEXT_COLOR_LIGHT} flex items-center gap-4`}>
            ...
            <div className="flex flex-wrap gap-4">
              {skills.map((skill) => (
                <motion.span
                  key={skill}
                  variants={itemVariants}
                  whileHover={{ scale: 1.07, boxShadow: `0 8px 16px rgba(244, 63, 94, 0.4)` }} // Enhanced shadow with accent color
                  className={`px-5 py-2.5 bg-black bg-opacity-30 border border-teal-600 ${TEXT_COLOR_LIGHT} rounded-full text-base font-medium cursor-pointer transition-all hover:border-${ACCENT_COLOR_RED} shadow-lg`}
                >
                  {skill}
                </motion.span>
              ))}
        ...
        {/* CTA - Matching button styles from homepage */}
        <motion.div
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-center gap-6">
            ...
            <motion.button
              onClick={() => navigate('/resume')} // Assuming you'll have a contact page
